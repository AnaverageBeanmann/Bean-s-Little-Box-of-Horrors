...
ENT.Contact 		= ""
ENT.Information		= "Projectile for Erectus; Spawns Fodder Voidlings"
...
--------------------
ENT.BLBOH_HorrorBall_GuyToSpawn = "npc_vj_blboh_forlorn"
ENT.BLBOH_HorrorBall_RageBall = false
--------------------
...
function ENT:Init()
	...
	if !self.BLBOH_HorrorBall_RageBall then
		if math.random(1,3) == 1 then
			self.BLBOH_HorrorBall_GuyToSpawn = "npc_vj_blboh_horror"
		end
	else
		self.BLBOH_HorrorBall_GuyToSpawn = "npc_vj_blboh_horror"
	end
...
function ENT:CustomOnRemove()
	...
	if self.BLBOH_HorrorBall_RageBall then
		...
		-- bloodeffect:SetColor(VJ_Color2Byte(Color(222,222,222,255)))
		bloodeffect:SetColor(VJ_Color2Byte(Color(0,0,0,255)))
	end
	bloodeffect:SetScale(150)
	bloodeffect:SetOrigin(self:GetPos() + self:GetRight()*65)
	util.Effect("VJ_Blood1",bloodeffect)
	bloodeffect:SetOrigin(self:GetPos() + self:GetRight()*-65)
	util.Effect("VJ_Blood1",bloodeffect)
	bloodeffect:SetOrigin(self:GetPos() + self:GetForward()*65)
	util.Effect("VJ_Blood1",bloodeffect)
	bloodeffect:SetOrigin(self:GetPos() + self:GetForward()*-65)
	util.Effect("VJ_Blood1",bloodeffect)
	bloodeffect:SetOrigin(self:GetPos() + self:GetUp()*30)
	util.Effect("VJ_Blood1",bloodeffect)
	bloodeffect:SetOrigin(self:GetPos() + self:GetUp()*-30)
	util.Effect("VJ_Blood1",bloodeffect)
	-- if self.BLBOH_HorrorBall_RageBall then
	...
	bloodeffect:SetOrigin(self:GetPos())
	util.Effect("VJ_Blood1",bloodeffect)
	VJ_EmitSound(self,{"ambient/machines/thumper_hit.wav"},80,math.random(40,50))
	VJ_EmitSound(self,{"ambient/machines/thumper_dust.wav"},80,math.random(70,80))
	VJ.EmitSound(self,{"ambient/machines/machine1_hit"..math.random(1,2)..".wav"},80,math.random(45,60))
	...
	if IsValid(horror) then
		...
		-- if self.BLBOH_HorrorBall_RageBall then
			-- horror.BLBOH_PreventSpawnSequence = true
		-- else
			horror.BLBOH_ForceSpawnSequence = true
		-- end
		horror:Spawn()
		timer.Simple(120, function() if IsValid(horror) then -- increase this?
			if horror:GetClass() == "npc_vj_blboh_forlorn" then
				horror.BLBOH_Forlorn_ScaleDamage = false
			end
			local d = DamageInfo()
